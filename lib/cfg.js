(function() {
  'use strict';
  var CND, PATH, alert, assign, badge, cast, cfg, debug, defaults, echo, help, info, isa, jr, key_infos, log, new_datom, package_json, rpr, type_of, types, urge, validate, warn, whisper;
  ...
  this._extract_job_settings = function(_job_settings) {
    ...
    for (k in defaults) {
      R[k] = (ref = _job_settings[k]) != null ? ref : defaults[k];
    }
  ...
  this.new_tagger = function(_job_settings) {
    ...
    debug('^3437776^', R);
    debug('^3336388^', this._extract_job_settings(_job_settings));
