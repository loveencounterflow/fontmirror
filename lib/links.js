(function() {
  'use strict';
  var CND, FS, PATH, alert, badge, cast, cfg, debug, defaults, echo, glob, help, info, isa, jr, log, mkdirp, package_json, rpr, trash, type_of, urge, validate, warn, whisper;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._list_links = function(path) {
    var x;
    return (function() {
      var i, len, ref, ref1, ref2, results;
      ref = glob.sync(PATH.join(path, '*'));
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        x = ref[i];
        if ((ref1 = (ref2 = this._lstat_safe(x)) != null ? ref2.isSymbolicLink() : void 0) != null ? ref1 : false) {
          results.push(x);
        }
      }
      return results;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._list_all_fontnicks = function() {
    /* TAINT pass in state object instead of re-computing */
    var FONTMIRROR, path_to_fontnicks, target_path, x;
    FONTMIRROR = require('..');
    target_path = FONTMIRROR.CFG.set_or_get('target_path');
    path_to_fontnicks = PATH.join(target_path, 'all');
    return (function() {
      var i, len, ref, results;
      ref = FONTMIRROR.LINKS._list_links(path_to_fontnicks);
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        x = ref[i];
        results.push(PATH.basename(x));
      }
      return results;
    })();
  };
